 ...
 * @experimental
 */
final class SymfonyBridgeNodeAnalyzer implements ContextualEscapingNodeAnalyzerInterface
{
    ...
    public function classify(Node $node): ?ContextualEscapingNodeType
    {
        return match ($node::class) {
            self::FORM_THEME_NODE => $this->isSupportedFormThemeNode($node) ? ContextualEscapingNodeType::ContextPreserving : null,
            self::TRANS_NODE => $this->isSupportedTransNode($node) ? ContextualEscapingNodeType::PlainTextOutput : null,
            default => null,
        };
    }
